...
//아래와 이름이 맞지 않으면 전혀 불러올 수 없으니 주의
//npm start
//npm i prop-types
...
const foodILike = [{
  id: 1,
  name:"Kimchi",
  image: "https://img.kr.news.samsung.com/kr/wp-content/uploads/2013/07/%EA%B9%80%EC%B9%98_03.jpg",
  rating: 5
},{
  id: 2,
  name:"Samgyeopsal",
  image: "https://img1.daumcdn.net/thumb/R800x0/?scode=mtistory2&fname=https%3A%2F%2Ft1.daumcdn.net%2Fcfile%2Ftistory%2F2431263958B8DA212C",
  rating: 4.8
},{
  id: 3,
  name: "Bibimbap",
...
function App2() {
  return (
    <div>
      ...
      {foodILike.map(dish=> (//dish 는 object이며, 단순히 배열(array)에 대한 변수값이다. 
      <Food 
      key={dish.id} 
      name={dish.name}
      picture={dish.image}
      rating={dish.rating}
      />//name의 경우 위에서 Food Function에서 Favorite라고 되어있던 것을 name으로 변경하여 적용하였는데,
      //단순히 변수의 이름이 아니라 연결되는 object이므로 맞춰서 써줘야 한다. dish.name의 경우는 foodILike의 name, image중에 변수 이름 하나
      //foreach의 경우와 비슷해보이는데 map은 return 값과 배열을 반환해주는 특징이 있다. foreach는 아무것도 안해서 이를 반환해주지 못하고 직접 지정해줘야 한다.
      //App()내에 다 넣어서 할 수도 있으나 로직이 너무 커지므로 나눠서 할것.
      ))}
      여기서부터는 renderFood로 직접 진행
